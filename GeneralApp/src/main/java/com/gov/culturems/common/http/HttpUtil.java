...
public class HttpUtil {
    ...
    public static void jsonRequest(final Context context, String url, RequestParams params, VolleyRequestListener listener) {
        VolleyRequest request = new VolleyRequest(Request.Method.POST, context, VersionController.getURLServer() + url + URLRequest.APPENDIX, params, listener);
    ...
    public static void jsonRequestGet(final Context context, String url, RequestParams params, VolleyRequestListener listener) {
        VolleyRequest request = new VolleyRequest(Request.Method.GET, context, VersionController.getURLServer() + url + URLRequest.APPENDIX + "?" + params.toString(), params, listener);
        request.setShouldCache(SHOULD_CACHE);
        request.setRetryPolicy(new DefaultRetryPolicy(
                MY_SOCKET_TIMEOUT_MS,
                DefaultRetryPolicy.DEFAULT_MAX_RETRIES,
