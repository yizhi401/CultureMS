    ...
    public void drawValues(Canvas c) {
        ...
        if (mChart.getLineData().getYValCount() < mChart.getMaxVisibleCount()
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                int valOffsetUp = (int) (dataSet.getCircleSize() * 1.75f);
                int valOffsetDown = (int) (dataSet.getCircleSize() * 2.0f);

                if (!dataSet.isDrawCirclesEnabled()) {
                    valOffsetUp = valOffsetUp / 2;
                    valOffsetDown = valOffsetDown / 2;
                }
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    if (entry.getData() instanceof ShouldDraw) {
                        if (!(((ShouldDraw) entry.getData()).shouldDrawValue)) {
                            continue;
                        } else {
                            float val = entry.getVal();
                            if ((((ShouldDraw) entry.getData()).shouldDrawUp)) {
