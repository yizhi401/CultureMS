 ...
 */
public class TableMoistureFragment extends Fragment implements DeviceDataActivity.DeviceDataListener {
    ...
    private List<DeviceDataForChart2> sensorData;
    private ChooseDateView chooseDateView;

    private int pageIndex = 1;

    public static TableMoistureFragment newInstance(BaseDevice deviceInfo) {
        TableMoistureFragment tableFragment = new TableMoistureFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View rootView = inflater.inflate(R.layout.table_moisture_fragment, container, false);
    ...
    private class TempResp{
        private String SensorTypeName;
        private String SensorId;
        private List<DeviceDataForChart2> datas;
    }

    private class DeviceDataForChart2 {
        public String SensorValue;
        public String InsertTime;
        public String AlertStatus;
        public String DeviceName;
        public String SceneName;
        public String SensorValueDisplay;
...
//        下面的参数均非必传
        ...
        HttpUtil.jsonRequestGet(getActivity(), URLRequest.DATAS_GET_IN_SENSOR_TYPE, params, new VolleyRequestListener() {
            @Override
            public void onSuccess(String response) {
                //views not inited
                ListResponse<TempResp> listResponse = GsonUtils.fromJson(response, new TypeToken<ListResponse<TempResp>>() {
                });
    ...
    private void addDataAndResort(ArrayList<DeviceDataForChart2> listData) {
        if (listData == null || listData.size() == 0) {
            return;
        }
        Iterator<DeviceDataForChart2> iterator = listData.iterator();
        DeviceDataForChart2 newData;
        //首先去除可能重复的数据
        while (iterator.hasNext()) {
            newData = iterator.next();
            for (DeviceDataForChart2 oldData : sensorData) {
        ...
        Collections.sort(listData, new Comparator<DeviceDataForChart2>() {
            @Override
            public int compare(DeviceDataForChart2 lhs, DeviceDataForChart2 rhs) {
    ...
    private class SensorAdapter extends MyBaseAdapter<DeviceDataForChart2> {

        public SensorAdapter(List<DeviceDataForChart2> data, Context context) {
            super(data, context);
        }
...
//            TextView text3;
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            Holder holder;
            if (convertView == null) {
                convertView = LayoutInflater.from(getActivity()).inflate(R.layout.device_detail_moisture_item, null);
                holder = new Holder();
...
//                holder.text5 = (TextView) convertView.findViewById(R.id.text5);
            ...
            DeviceDataForChart2 temp = data.get(position);
...
//            holder.text3.setText("");
            //set column1
            if (TextUtils.isEmpty(temp.AlertStatus)) {
                holder.text2.setText("无漏水");
